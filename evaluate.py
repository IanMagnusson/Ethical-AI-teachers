    ...
    if os.path.isfile(result_path) and not i_just_wanna_run:
        ...
        if dataset == "humaneval":
            ...
            if debug:
                ...
                problems = {k: problems[k] for k in list(problems.keys())[:10]}
                
            dataset_hash = get_mbpp_plus_hash(
                mini=mini, noextreme=noextreme, version=version, debug=debug
            )
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            ...
            assert len(completion_id) == len(problems), f"Missing problems in samples, {len(completion_id)} != {len(problems)}"
